...
export default function GoToNextYearButton(props)
{
    ...
    let can_go_to_next_year = false;
    if(state.current_year === ((new Date(state.upper_bound)).getFullYear() - 1))
    {
        if(state.current_month <= (new Date(state.upper_bound)).getMonth())
        {
            can_go_to_next_year = true;
        }
    }
    else if(state.current_year < ((new Date(state.upper_bound)).getFullYear() - 1))
    {
        can_go_to_next_year = true;
    }

    return (
        <button id='go_to_next_year_button' className={`go_to_next_year_button ${(!can_go_to_next_year) ? 'go_to_next_year_button_disabled' : ''}`} aria-label='go_to_next_year_button' data-test-id='go_to_next_year_button' onClick={() => {dispatcher({type: 'go_to_next_year'})}} disabled={!can_go_to_next_year}>
            {'>>'}
        </button>
    );
}