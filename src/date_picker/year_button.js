...
export default function YearButton(props)
{
    ...
    return (
        <button id={`year_button_${props.year}`} className={`year_button ${(state.current_year === props.year) ? 'year_button_selected' : ''} ${disable_year_button ? 'year_button_disabled' : ''}`} aria-label={`year_button_${props.year}`} data-test-id={`year_button_${props.year}`} onClick={() => {dispatcher({type:'go_to_year',year: props.year, month: props.month})}} disabled={disable_year_button}>
            {props.year}
        </button>
    );
}
