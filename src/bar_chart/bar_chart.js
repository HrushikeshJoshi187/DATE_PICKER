    ...
    for(let i = 0, j = 0; i < number_of_columns_in_chart + number_of_missing_dates; i++)
    {
        if(!missing_dates.has(data[i].date))
        {
            ...
            columns_in_the_chart.push(
                <span key={j} id={`column_${data[i].date}`} className={`column ${state.selected_dates.has(data[i].date) ? 'column_selected' : ''}`} aria-label={`column_${data[i].date}`} data-test-id={`column_${data[i].date}`} style={
                    {
                        height: `${data[i].sales}%`, 
